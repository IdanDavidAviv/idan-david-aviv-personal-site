            ...
            <svg width="200" height="140" viewBox="0 0 200 140" fill="none" className="relative z-10">
                ...
                {pulses.flatMap(pulse => {
                    ...
                    for (let i = 0; i < pulse.route.length - 1; i++) {
                        const start = PENTAGON_NODES.find(n => n.id === pulse.route[i])!
                        const end = PENTAGON_NODES.find(n => n.id === pulse.route[i+1])!
                        
                        const dist = Math.hypot(end.x - start.x, end.y - start.y);
                        // Orbital paths can be slightly faster to emphasize cyclical flow
                        const segmentDuration = dist * 0.005; 
